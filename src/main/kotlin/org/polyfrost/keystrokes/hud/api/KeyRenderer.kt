...
object KeyRenderer {
    ...
    @JvmStatic
    fun drawBackground(x: Float, y: Float, width: Float, height: Float, style: KeyStyle, pressed: Boolean, fade: Float, scale: Float) {
        val color = style.backgroundColor(pressed, fade)

        if (style.isRounded) {
            val radius = style.cornerRadius * scale
            NanoVGHelper.INSTANCE.setupAndDraw(true) { vg: Long ->
    ...
    @JvmStatic
    fun drawCenteredText(fr: FontRenderer, text: String?, x: Float, y: Float, width: Float, height: Float, style: KeyStyle, pressed: Boolean, fade: Float) {
    ...
    fun drawSpaceGlyph(x: Float, y: Float, width: Float, height: Float, color: Int, shadow: Boolean) {
        ...
        drawHorizontalLine(cx - 6, cx + 5, cy - 0.5f, adjustedColor)

        if (shadow) {
            if ((adjustedColor and -0x4000000) == 0) {
                adjustedColor = adjustedColor or -0x1000000
            }
            val shadowColor = (adjustedColor and 0xFCFCFC) shr 2 or (adjustedColor and -0x1000000)
            drawHorizontalLine(cx - 5, cx + 6, cy + 0.5f, shadowColor)
        }
    }

    private fun drawHorizontalLine(startX: Float, endX: Float, y: Float, color: Int) {
        val (sx, ex) = if (endX < startX) endX to startX else startX to endX
        Platform.getGLPlatform().drawRect(sx, y, ex + 1, y + 1, color)
    }
}
